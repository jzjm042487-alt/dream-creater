...
四档难度共用同一个敌人内容对象。胜率目标以标准难度作为硬验收门。
其他难度只通过相同内容对象、确定性、候选截断、搜索深度和随机游标规则
硬验收；胜率差异在本 MVP 阶段作为报告型观察项，不作为逐场失败条件。
...
若报告差异超出范围，后续只能调整搜索宽度、候选随机或评价权重；不得给
难度添加属性倍率。等主动蛊、地形和高阶敌人动作池变丰富后，QA 可以把
聚合差异重新提升为硬门；在当前 13 场 MVP 动作池内，不要求每场战斗都
独立满足该差异区间。
...
- 同一 `15` 案例矩阵满足标准难度胜率、回合、撤退和压力预算；第 14.5
  节难度差异区间输出为报告，不作为本 Wave 1 阻断项。
