...
旧版按日期连续播放的六个剧本文件已经删除；本表中的十个文件是唯一生产版本。
...
选择若有资源、证据或准备门槛，在 NPC 开场后按 `选择 X 可用条件：<predicate>` 逐项登记。
条件不满足时该选择显示为锁定，并显示谓词对应的本地化原因；`always` 表示始终可选。可用条件
只控制选项开放，不代替属性判定的成功与失败分支。
